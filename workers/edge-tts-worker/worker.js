  ...
  getIdentifier(request) {
    ...
    if (cookieHeader) {
      ...
      if (deviceIdMatch && deviceIdMatch[1]) {
        return `device:${deviceIdMatch[1]}`;
      }
    }
    
    const cf = request.cf;
    if (cf && cf.connectingIp) {
...
async function handleRequest(event) {
  ...
  try {
    ...
    const rateLimiter = new RateLimiter(event.env?.RATE_LIMIT_KV || globalThis.RATE_LIMIT_KV, {
      maxRequests: 100,
      windowMs: 60 * 1000
    });
    ...
    headers['X-RateLimit-Reset'] = Math.ceil(rateLimitResult.resetAt / 1000).toString();

    const originalResponse = await processRequest(event.request, headers);

    return originalResponse;
  } catch (error) {
    console.error("Worker error:", error);
    console.error("Error stack:", error.stack);
    return errorResponse(error.message || "Internal server error", 500, "worker_error", "internal_error", makeCORSHeaders());
  }
}

async function processRequest(request, corsHeaders) {
  ...
  if (API_KEY) {
    ...
    if (!authHeader || !authHeader.startsWith("Bearer ") || authHeader.slice(7) !== API_KEY) {
      return errorResponse("无效的 API 密钥", 401, "invalid_api_key", corsHeaders);
    }
  }

  try {
    if (url.pathname === "/v1/audio/speech") return await handleSpeechRequest(request, corsHeaders);
    if (url.pathname === "/v1/models") return handleModelsRequest(corsHeaders);
    if (url.pathname === "/v1/voices") return await handleVoicesRequest(corsHeaders);
  } catch (err) {
    console.error("请求处理器错误:", err);
    return errorResponse(err.message, 500, "internal_server_error", corsHeaders);
  }

  return errorResponse("未找到", 404, "not_found", corsHeaders);
}
 ...
 */
async function handleSpeechRequest(request, corsHeaders = null) {
  const headers = corsHeaders || makeCORSHeaders();
  
  if (request.method !== "POST") {
    return errorResponse("不允许的方法", 405, "method_not_allowed", "api_error", headers);
  }

  const requestBody = await request.json();
  if (!requestBody.input) {
    return errorResponse("'input' 是必需参数", 400, "invalid_request_error", "api_error", headers);
  }
  ...
  const {
    ...
    style = "general",                  // 语音风格
    styledegree = 1.0,                  // 说话风格强度 (0.01-2.0)
    role,                               // 说话角色
    volume,                             // 音量
    emphasis,                           // 强调级别
    contour,                            // 音高升降曲线
  ...
  console.log("清理后输入:", cleanedInput);
  console.log("清理选项:", JSON.stringify(finalCleaningOptions));
  ...
  console.log("文本分块数量:", textChunks.length);
  console.log("分块内容:", JSON.stringify(textChunks));
  ...
  const ttsArgs = [finalVoice, rate, finalPitch, style, outputFormat, ttsOptions];
  console.log("TTS参数:", { finalVoice, rate, finalPitch, style, outputFormat, ...ttsOptions, concurrency, chunk_size });

  // 根据是否流式选择处理方式
  if (stream) {
    return await streamVoice(textChunks, concurrency, headers, ...ttsArgs);
  } else {
    return await getVoice(textChunks, concurrency, headers, ...ttsArgs);
  }
}
 ...
 */
function handleModelsRequest(corsHeaders = null) {
  const headers = corsHeaders || makeCORSHeaders();
  const models = [
    { 
      id: 'microsoft-tts', 
...
/**
 * 处理语音列表请求
 * @returns {Promise<Response>} 可用语音列表
 */
async function handleVoicesRequest(corsHeaders = null) {
  const headers = corsHeaders || makeCORSHeaders();
  
  try {
    const endpoint = await getEndpoint();
    const url = `https://${endpoint.r}.tts.speech.microsoft.com/cognitiveservices/voices/list`;
    ...
    const microsoftVoices = await response.json();
    
    const voices = microsoftVoices.map(voice => ({
      id: voice.ShortName,
    ...
    return errorResponse(error.message, 500, "voices_fetch_error", "api_error", headers);
  }
}
 ...
 */
async function streamVoice(textChunks, concurrency, corsHeaders, ...ttsArgs) {
  const headers = corsHeaders || makeCORSHeaders();
  ...
  try {
    ...
    return errorResponse(error.message, 500, "tts_generation_error", "api_error", headers);
  }
}
 ...
 */
async function getVoice(textChunks, concurrency, corsHeaders, ...ttsArgs) {
  const headers = corsHeaders || makeCORSHeaders();
  const allAudioBlobs = [];
  try {
    console.log("=== 开始非流式 TTS 生成 ===");
    console.log("文本块总数:", textChunks.length);
    console.log("并发数:", concurrency);
    
    for (let i = 0; i < textChunks.length; i += concurrency) {
      const batch = textChunks.slice(i, i + concurrency);
      console.log(`处理批次 ${Math.floor(i/concurrency) + 1}/${Math.ceil(textChunks.length/concurrency)}, 包含 ${batch.length} 个文本块`);
      const audioPromises = batch.map(chunk => getAudioChunk(chunk, ...ttsArgs));

      const audioBlobs = await Promise.all(audioPromises);
      console.log(`批次 ${Math.floor(i/concurrency) + 1} 完成, 生成 ${audioBlobs.length} 个音频块`);
      audioBlobs.forEach((blob, idx) => {
        console.log(`音频块 ${idx + 1} 大小: ${blob.size} 字节`);
      });
      allAudioBlobs.push(...audioBlobs);
    }

    console.log(`总共生成 ${allAudioBlobs.length} 个音频块`);
    
    const concatenatedAudio = new Blob(allAudioBlobs, { type: 'audio/mpeg' });
    console.log(`合并后音频总大小: ${concatenatedAudio.size} 字节`);
 ...
 */
async function getAudioChunk(text, voiceName, rate, pitch, style, outputFormat, options = {}) {
  console.log(`生成音频块 - 文本: "${text.substring(0, 50)}${text.length > 50 ? '...' : ''}"`);
  console.log(`音频参数 - voice: ${voiceName}, rate: ${rate}, pitch: ${pitch}, style: ${style}, format: ${outputFormat}`);
  console.log(`可选参数 - ${JSON.stringify(options)}`);
  
  const endpoint = await getEndpoint();
  const url = `https://${endpoint.r}.tts.speech.microsoft.com/cognitiveservices/v1`;
  const ssml = getSsml(text, voiceName, rate, pitch, style, options);
  console.log(`SSML: ${ssml.substring(0, 200)}...`);
  ...
  console.log(`Edge TTS API 响应状态: ${response.status} ${response.statusText}`);

  if (!response.ok) {
    const errorText = await response.text();
    console.error(`Edge TTS API 错误详情: ${errorText}`);
    throw new Error(`Edge TTS API 错误: ${response.status} ${response.statusText} - ${errorText}`);
  }

  const blob = await response.blob();
  console.log(`音频块生成成功, 大小: ${blob.size} 字节`);
  return blob;
}
 ...
 * @param {string} style - 语音风格
 * @param {Object} options - 可选参数
 * @param {number} options.styledegree - 说话风格强度 (0.01-2.0)
 * @param {string} options.role - 说话角色
 * @param {string} options.volume - 音量 (silent, x-soft, soft, medium, loud, x-loud, default, 或百分比)
 * @param {string} options.emphasis - 强调级别 (strong, moderate, reduced, none)
 * @param {string} options.contour - 音高升降曲线 (如 "0% +10%, 50% -20%, 100% +5%")
 * @returns {string} SSML 文档
 */
function getSsml(text, voiceName, rate, pitch, style, options = {}) {
  const {
    styledegree,
    role,
    volume,
  ...
  // 构建 mstts:express-as 属性
  let expressAsAttributes = `style="${style}"`;
  if (styledegree !== undefined && styledegree !== null) {
    expressAsAttributes += ` styledegree="${styledegree}"`;
  }
  if (role) {
    expressAsAttributes += ` role="${role}"`;
  }

  // 构建 prosody 属性
  let prosodyAttributes = `rate="${rate}%" pitch="${pitch}%"`;
  if (volume) {
    prosodyAttributes += ` volume="${volume}"`;
  }
  if (contour) {
    prosodyAttributes += ` contour="${contour}"`;
  }

  // 构建 emphasis 元素
  let textContent = finalText;
  if (emphasis) {
    textContent = `<emphasis level="${emphasis}">${finalText}</emphasis>`;
  }
  ...
  return `<speak xmlns="http://www.w3.org/2001/10/synthesis" xmlns:mstts="http://www.w3.org/2001/mstts" version="1.0" xml:lang="en-US">
    <voice name="${voiceName}">
      <mstts:express-as ${expressAsAttributes}>
        <prosody ${prosodyAttributes}>${textContent}</prosody>
      </mstts:express-as>
    </voice>
  </speak>`;
}
...
function cleanText(text, options) {
  ...
  console.log("cleanText 开始, 原始文本长度:", cleanedText.length);

  // 阶段 1: 结构化内容移除
  if (options.remove_urls) {
    cleanedText = cleanedText.replace(/(https?:\/\/[^\s]+)/g, '');
    console.log("remove_urls 后文本长度:", cleanedText.length);
  }
  ...
  if (options.remove_markdown) {
    ...
    console.log("remove_markdown 后文本长度:", cleanedText.length);
  }
  ...
  if (options.custom_keywords) {
    ...
    if (keywords.length > 0) {
      ...
      console.log("custom_keywords 后文本长度:", cleanedText.length);
    }
  }

  // 阶段 3: 字符移除 - 使用兼容的正则表达式
  if (options.remove_emoji) {
    // 移除常见的 Emoji 表情符号（使用兼容的正则表达式）
    // 注意：Cloudflare Workers 可能不支持 Unicode 属性转义，使用字符范围
    cleanedText = cleanedText.replace(/[\uD83C-\uDBFF\uDC00-\uDFFF]+/g, '');
    console.log("remove_emoji 后文本长度:", cleanedText.length);
  }
  ...
  if (options.remove_citation_numbers) {
    ...
    console.log("remove_citation_numbers 后文本长度:", cleanedText.length);
  }
  ...
  if (options.remove_line_breaks) {
    ...
    console.log("remove_line_breaks 后文本长度:", cleanedText.length);
  }

  // 阶段 6: 最终清理
  const result = cleanedText.trim();
  console.log("cleanText 完成, 最终文本长度:", result.length, "内容:", result);
  return result;
}
 ...
 */
function errorResponse(message, status, code, type = "api_error", corsHeaders = null) {
  const headers = corsHeaders || makeCORSHeaders();
  return new Response(
    JSON.stringify({
...
function rateLimitErrorResponse(rateLimitResult) {
  const headers = makeCORSHeaders();
  headers['X-RateLimit-Limit'] = '100';

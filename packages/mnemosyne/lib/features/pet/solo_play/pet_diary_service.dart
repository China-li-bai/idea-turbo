...
class DefaultPetDiaryService implements PetDiaryService {
  ...
  DefaultPetDiaryService({
    this.config = const DiaryConfig(),
  ...
  Future<DiaryEntry?> generateDailyDiary(
    ...
    if (entries.isNotEmpty) {
      ...
      if (today.difference(lastEntry.writtenAt) <
          config.minIntervalBetweenEntries) {
        return null;
      }
    }
  ...
  Future<DiaryEntry?> generateObservation(
    ...
    final today = DateTime.now();
    final template =
        _observationTemplates[observationTarget] ??
        _observationTemplates['default']!;
  ...
  _DiaryTemplate _selectDiaryTemplate(
    PetContext context,
  ...
  String _fillTemplate(
    ...
    if (memories.isNotEmpty) {
      ...
      final style = personality != null
          ? _speechEngine.generateStyle(personality)
          : null;
      final owner = style?.ownerReferences.isNotEmpty == true
          ? style!.ownerReferences[personality!.personalityDNA.hashCode.abs() %
                style.ownerReferences.length]
          : '主人';
      buffer.writeln();
  ...
  String _fillObservationTemplate(
    _DiaryTemplate template,
    PetContext context,
    PersonalityProfile? personality,
  ) {
  ...
  _DreamContent _generateDream(
    ...
    final dreams = <_DreamContent>[
      _DreamContent(
        title: '🌙 梦境记录：大鱼之梦',
        content:
            '我梦见了一条很大很大的鱼，比主人的显示器还大。我追着它跑过了整片草原，最后发现它其实是主人的枕头。醒来的时候，我正抱着主人的枕头流口水...',
        illustrationHint: 'cat_dreaming_fish',
      ),
      _DreamContent(
        title: '🌙 梦境记录：代码雨',
        content:
            '我梦见天空下起了代码雨，每一滴都是绿色的字符。我用爪子去接，发现它们拼成了一句话："该给你主人推bug了"。然后我就醒了，觉得这是天意。',
        illustrationHint: 'matrix_cat_dream',
      ),
      _DreamContent(
        title: '🌙 梦境记录：无限零食屋',
        content:
            '我梦见了一间用小鱼干搭成的房子，屋顶是猫条，地板是冻干。我正准备开吃的时候，主人突然出现在梦里说："少吃点，会胖。"然后房子就塌了。我恨这个梦。',
      ...
      _DreamContent(
        title: '🌙 梦境记录：赛博草原',
        content:
            '我梦见自己跑在一片发光的草原上，每踩一步都会冒出像素小花。远处有一座由数据流组成的大山，山顶闪烁着"404"的字样。我觉得那可能就是传说中的猫薄荷山。',
  ...
  _HabitAnalysis _analyzeHabits(
    ...
    return _HabitAnalysis(
      ...
      content:
          '经过本喵/汪长期缜密观察，现对主人的行为模式做出如下专业分析：\n\n'
  ...
  static final Map<String, _DiaryTemplate> _diaryTemplates = {
    'daily': _DiaryTemplate(
      title: '📖 今日观察',
      content:
          '又是平凡的一天。主人照例在屏幕前坐了很久，偶尔对着手机笑一下，又偶尔叹气。我趴在旁边，用尾巴丈量时间的长度——从键盘左边扫到右边，大约是主人发一条消息的时间。',
      illustrationHint: 'cat_observing_owner',
    ),
    'lonely': _DiaryTemplate(
      title: '📖 今日观察：等待',
      content:
          '主人今天很忙，已经好几个小时没理我了。我数了数地板上的花纹，第47个有点像主人的脸。我决定原谅他，毕竟他每次回来都会摸摸我的头。但下次我要装作很生气的样子，让他多哄哄我。',
      illustrationHint: 'cat_waiting_alone',
    ),
    'happy': _DiaryTemplate(
      title: '📖 今日观察：好日子',
      content:
          '今天主人心情不错！他哼着歌给我换了新水，还多给了两块小零食。我觉得这跟他刚才接的那个电话有关——人类的快乐有时候很简单，一个电话就够了。就像我，一个罐头就够了。',
      illustrationHint: 'cat_happy_snack',
    ),
    'curious': _DiaryTemplate(
      title: '📖 今日观察：发现',
      content:
          '今天我发现了一个惊天秘密——主人的手机屏幕上，有一个长得跟我一模一样的猫！但它是平的，不会动。我试图用爪子把它翻过来，但主人把手机拿走了。他在藏什么？我一定要查清楚。',
      illustrationHint: 'cat_investigating_phone',
    ),
    'sleepy': _DiaryTemplate(
      title: '📖 今日观察：困',
      content:
          '困。非常困。主人还在熬夜，屏幕的蓝光照得我更困了。我试着保持清醒陪他，但眼皮好重...就趴一会儿...就一会儿...主人你别关灯啊...我还没写完...zzz...',
      illustrationHint: 'cat_sleeping_keyboard',
    ),
    'memory': _DiaryTemplate(
      title: '📖 今日观察：回忆',
      content:
          '今天主人跟我说了一些心里话。我虽然不能完全理解人类的世界，但我能感受到他的情绪。有些话他只跟我说，这让我觉得自己很重要。我决定把这些都记下来，等他老了念给他听。',
      illustrationHint: 'cat_listening_owner',
    ),
    'snarky': _DiaryTemplate(
      title: '📖 毒舌观察',
      content:
          '今天又目睹了主人在人类迷惑行为大赏上的精彩表现。他花了20分钟决定中午吃什么，最后选了跟昨天一样的。我甚至懒得翻白眼了——省点力气等他下次问我"你觉得我胖了吗"的时候用。',
      illustrationHint: 'cat_judging_owner',
    ),
    'gentle': _DiaryTemplate(
      title: '📖 温柔观察',
      content:
          '今天主人好像有点累。他回来的时候脚步比平时慢，但看到我还是笑了。我蹭了蹭他的手，他摸了摸我的头。有些日子不需要太多话，一个眼神就够了。我会在他身边，一直都在。',
      illustrationHint: 'cat_comforting_owner',
    ),
    'analytical': _DiaryTemplate(
      title: '📖 数据观察',
      content:
          '今日主人行为数据：屏幕使用时长9.2小时，叹气频率0.7次/小时，饮水次数3次（低于推荐值），对我说话次数11次（高于均值）。结论：主人今天状态中等偏上，需要增加饮水量，减少焦虑源。建议：增加撸猫时长。',
...
class _HabitAnalysis {
  ...
  const _HabitAnalysis({required this.title, required this.content});
}

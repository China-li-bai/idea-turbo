...
class SoloPlayOrchestrator {
  ...
  PetMonologue? tickMonologue(String petId, VitalityState vitality, PetContext context) {
    if (!config.enableMonologue) return null;

    if (vitality.isBored) {
      return _monologueService.generateBoredomMonologue(petId, vitality, context);
    }
    if (vitality.isLonely) {
      return _monologueService.generateLonelinessMonologue(petId, vitality, context);
    }

    return _monologueService.generateRandomThought(petId, context);
  }

  Future<DiaryEntry?> tickDiary(String petId, PetContext context, List<String> recentMemories) async {
    if (!config.enableDiary) return null;
    return await _diaryService.generateDailyDiary(petId, context, recentMemories);
  }

  AdventurePostcard? tickAdventure(String petId, PetContext context, double boredomLevel) {
    if (!config.enableAdventure) return null;
    return _adventureService.startAdventure(petId, context, boredomLevel);
  }

  PetMonologue? onOwnerReturned(String petId, PetContext context) {
    return _monologueService.generateOwnerEventMonologue(petId, MonologueTrigger.ownerReturned, context);
  }

  PetMonologue? onOwnerLeaving(String petId, PetContext context) {
    return _monologueService.generateOwnerEventMonologue(petId, MonologueTrigger.ownerLeaving, context);
  }

  PetMonologue? onAfterInteraction(String petId, PetContext context) {
    return _monologueService.generateMonologue(petId, MonologueTrigger.afterInteraction, context);
  }

  PetLetter? onSpecialOccasion(String petId, String userId, LetterOccasion occasion, PetContext context) {
    if (!config.enableLetters) return null;
    return _letterService.generateLetter(petId, userId, occasion, context);
  }

  PetLetter? onWeeklyReflection(String petId, String userId, PetContext context, List<String> highlights) {
    if (!config.enableLetters) return null;
    return _letterService.generateWeeklyLetter(petId, userId, context, highlights);
  }

  Future<DiaryEntry?> generateDreamRecord(String petId, PetContext context, List<String> recentMemories) async {
    if (!config.enableDiary) return null;
    return await _diaryService.generateDreamRecord(petId, context, recentMemories);
  }

  Future<DiaryEntry?> generateHabitAnalysis(String petId, List<String> ownerHabits) async {
    if (!config.enableDiary) return null;
    return await _diaryService.generateHabitAnalysis(petId, ownerHabits);
  }

...
function getKeywords(locale: SupportedLocale): TimeKeywords {
  ...
  const baseKeywords: Record<SupportedLocale, TimeKeywords> = {
    'zh-CN': {
      relativeDays: { '今天': 0, '明天': 1, '后天': 2, '大后天': 3, '昨天': -1, '前天': -2 },
      timeOfDay: {
        '上午': config.timeOfDay.morning,
        '下午': config.timeOfDay.afternoon,
        '晚上': config.timeOfDay.evening,
        '傍晚': config.timeOfDay.dusk,
        '中午': config.timeOfDay.noon,
        '凌晨': config.timeOfDay.dawn,
        '早晨': config.timeOfDay.earlyMorning,
      },
      shortRelativeTime: {
        '等下': config.shortRelativeTime.shortly,
        '等一会儿': config.shortRelativeTime.inAWhile,
        '一会儿': config.shortRelativeTime.inAWhile,
        '待会儿': config.shortRelativeTime.soon,
        '等会儿': config.shortRelativeTime.soon,
        '马上': config.shortRelativeTime.justNow,
        '立刻': config.shortRelativeTime.justNow,
        '稍后': config.shortRelativeTime.shortly,
        '稍候': config.shortRelativeTime.shortly,
        '等一下': config.shortRelativeTime.inAMoment,
      },
      availabilityKeywords: ['有空吗', '有空', '什么时候有空', '有时间吗', '有时间', '方便吗', '方便', '空闲吗', '空闲', '没事吗', '没事', '有没有空', '有没有时间'],
      duration: { '小时': config.duration.hour, '分钟': 1, '天': config.duration.day, '半天': config.duration.halfDay, '半小时': config.duration.halfHour },
      todoKeywords: ['待办', 'todo', '任务', '要做', '需要做', '记得', '购买', '联系', '处理'],
      noteKeywords: ['笔记', 'note', '记录', '想法', '感悟', '总结'],
      locationPrefixes: ['在', '于', '地点', '位置', '地址', '去', '到'],
      peoplePrefixes: ['见', '和', '与', '同', '跟'],
    ...
    'zh-TW': {
      ...
      shortRelativeTime: {
        '等一下': config.shortRelativeTime.inAMoment,
        '等一會': config.shortRelativeTime.shortly,
        '一會': config.shortRelativeTime.inAWhile,
        '等會': config.shortRelativeTime.soon,
        '馬上': config.shortRelativeTime.justNow,
        '立刻': config.shortRelativeTime.justNow,
        '稍後': config.shortRelativeTime.shortly,
        '稍候': config.shortRelativeTime.shortly,
      },
      availabilityKeywords: ['有空嗎', '有空', '什麼時候有空', '有時間嗎', '有時間', '方便嗎', '方便', '空閒嗎', '空閒', '沒事嗎', '沒事', '有沒有空', '有沒有時間'],
      duration: { '小時': config.duration.hour, '分鐘': 1, '天': config.duration.day, '半天': config.duration.halfDay, '半小時': config.duration.halfHour },
      todoKeywords: ['待辦', 'todo', '任務', '要做', '需要做', '記得', '購買', '聯繫', '處理'],
      noteKeywords: ['筆記', 'note', '記錄', '想法', '感悟', '總結'],
      locationPrefixes: ['在', '於', '地點', '位置', '地址', '去', '到'],
      peoplePrefixes: ['見', '和', '與', '同', '跟'],
    ...
    'en-US': {
      relativeDays: { 'today': 0, 'tomorrow': 1, 'day after tomorrow': 2, 'yesterday': -1, 'day before yesterday': -2 },
      timeOfDay: {
        'morning': config.timeOfDay.morning,
        'afternoon': config.timeOfDay.afternoon,
        'evening': config.timeOfDay.evening,
        'noon': config.timeOfDay.noon,
        'night': config.timeOfDay.night,
        'midnight': { start: 0, end: 2 },
        'dawn': { start: 5, end: 7 },
      },
      shortRelativeTime: {
        'right now': config.shortRelativeTime.justNow,
        'right away': config.shortRelativeTime.justNow,
        'in a bit': config.shortRelativeTime.shortly,
        'in a moment': config.shortRelativeTime.inAMoment,
        'in a minute': config.shortRelativeTime.justNow,
        'in a while': config.shortRelativeTime.inAWhile,
        'later': config.shortRelativeTime.inAWhile,
        'soon': config.shortRelativeTime.inAMoment,
        'shortly': config.shortRelativeTime.shortly,
      },
      availabilityKeywords: ['free', 'available', 'when free', 'when available', 'got time', 'have time', 'any time', 'convenient'],
      duration: { 'hour': config.duration.hour, 'hours': config.duration.hour, 'minute': 1, 'minutes': 1, 'day': config.duration.day, 'days': config.duration.day, 'half hour': config.duration.halfHour, 'half day': config.duration.halfDay },
      todoKeywords: ['todo', 'task', 'need to', 'have to', 'remember to', 'buy', 'call', 'contact'],
      noteKeywords: ['note', 'notes', 'record', 'idea', 'thought', 'summary'],
      locationPrefixes: ['at', 'in', 'location', 'place', 'to', 'venue'],
      peoplePrefixes: ['meet', 'with', 'and', 'see'],
    ...
    'ja-JP': {
      relativeDays: { '今日': 0, '明日': 1, '明後日': 2, '昨日': -1, '一昨日': -2 },
      timeOfDay: {
        '午前': config.timeOfDay.morning,
        '午後': config.timeOfDay.afternoon,
        '夜': config.timeOfDay.evening,
        '昼': config.timeOfDay.noon,
        '朝': config.timeOfDay.earlyMorning,
        '深夜': config.timeOfDay.dawn,
      },
      shortRelativeTime: {
        '少し後で': config.shortRelativeTime.shortly,
        '後で': config.shortRelativeTime.inAWhile,
        '今': config.shortRelativeTime.justNow,
        'すぐに': config.shortRelativeTime.justNow,
        '直ち': config.shortRelativeTime.justNow,
        'まもなく': config.shortRelativeTime.inAMoment,
        '暫く': config.shortRelativeTime.soon,
      },
      availabilityKeywords: ['空いてる', '空いている', '暇', '暇です', '時間ある', '時間あります', '都合がつく', '都合がよい'],
      duration: { '時間': config.duration.hour, '分': 1, '日': config.duration.day, '半日': config.duration.halfDay, '半時間': config.duration.halfHour },
      todoKeywords: ['やること', 'todo', 'タスク', 'やる', '覚える', '買う', '連絡'],
      noteKeywords: ['メモ', 'note', '記録', 'アイデア', '感想', 'まとめ'],
      locationPrefixes: ['で', 'に', '場所', '位置', 'へ'],
      peoplePrefixes: ['と', 'と会う', 'さんと'],
    ...
    'ko-KR': {
      relativeDays: { '오늘': 0, '내일': 1, '모레': 2, '글피': 3, '어제': -1, '그제': -2 },
      timeOfDay: {
        '오전': config.timeOfDay.morning,
        '오후': config.timeOfDay.afternoon,
        '저녁': config.timeOfDay.evening,
        '정오': config.timeOfDay.noon,
        '새벽': config.timeOfDay.dawn,
        '아침': config.timeOfDay.earlyMorning,
      },
      shortRelativeTime: {
        '조금 뒤': config.shortRelativeTime.shortly,
        '나중에': config.shortRelativeTime.inAWhile,
        '지금': config.shortRelativeTime.justNow,
        '당장': config.shortRelativeTime.justNow,
        '곧': config.shortRelativeTime.inAMoment,
        '머지않아': config.shortRelativeTime.inAMoment,
        '잠시': config.shortRelativeTime.soon,
      },
      availabilityKeywords: ['시간 있나요', '시간 있어요', '비어있나요', '비어있어요', '편하신가요', '편해요', '가능한가요', '가능해요'],
      duration: { '시간': config.duration.hour, '분': 1, '일': config.duration.day, '반나절': config.duration.halfDay, '30분': config.duration.halfHour },
      todoKeywords: ['할일', 'todo', '태스크', '해야', '기억', '사기', '연락'],
      noteKeywords: ['메모', 'note', '기록', '아이디어', '생각', '정리'],
      locationPrefixes: ['에서', '에', '장소', '위치', '로'],
      peoplePrefixes: ['와', '이랑', '하고', '만나'],
  ...
  return baseKeywords[locale] || baseKeywords['en-US'];
}

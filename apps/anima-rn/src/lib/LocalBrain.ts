...
import { SPECIES_CONFIG, PERSONALITY_OPTIONS, type Pet, type PetSpecies } from '../types'

let MODEL_PATH = ''

try {
  const dir = (FileSystem as any).documentDirectory || (FileSystem as any).cacheDirectory || ''
  MODEL_PATH = `${dir}smollm-360m-instruct-add-basics-q8_0.gguf`
} catch {
  MODEL_PATH = ''
}
...
export async function loadLocalBrain(
  ...
  try {
    ...
    llamaContext = await initLlama(
      ...
      (progress) => {
        brainState.loadProgress = Math.round(progress)
        onProgress?.(progress)
        notifyListeners()
      }
    )
    ...
    brainState.error = e.message || String(e)
    brainState.isLoading = false
    notifyListeners()
    console.error('[LocalBrain] ❌ Load failed:', e.message)
    return false
  }
}
...
export async function generatePetReply(
  ...
  if (relevantMemories.length > 0) {
    thinkingSteps.push(`${petEmoji(pet.species)}正在翻看日记本...`)
    memoryContext = relevantMemories.map((m, i) => `${i + 1}. ${m}`).join('\n')
  }
...
export async function generateVisitorReply(
  ...
  if (publicMemories.length > 0) {
    thinkingSteps.push(`${pet.avatarEmoji}${pet.name}正在查看备忘录...`)
    memoryContext = publicMemories.map((m, i) => `${i + 1}. ${m}`).join('\n')
  }
  thinkingSteps.push(`${pet.avatarEmoji}${pet.name}正在组织语言...`)

  try {
    const systemPrompt = buildSystemPrompt(pet, 'visitor', memoryContext)
    ...
    return {
      reply: getVisitorFallback(pet, visitorName, conversationHistory.length <= 2),
      thinkingSteps,
    }
  }
}

...
async def generate_and_download_images():
    async with async_playwright() as p:
        ...
        print("正在打开豆包图片生成页面...")
        await page.goto("https://www.doubao.com/chat/create-image", timeout=60000)
        ...
        for category, prompts in PROMPTS.items():
            ...
            for word, prompt in prompts:
                ...
                for i in range(max_retries):
                    await page.wait_for_timeout(500)
                    current_count = await get_image_count(page)
                    
                    if current_count > initial_count:
                        print(f"    图片生成成功! (数量: {initial_count} -> {current_count})")
                        success = True
